...
def get_products():
    ...
    result = [{'id': product.id, 'name': product.name, 'price': str(product.price), 'stock': product.stock} for product in products]
...
def get_product(product_id):
    ...
    return jsonify({'id': product.id, 'name': product.name, 'price': str(product.price), 'stock': product.stock})

@product_controller.route('/api/products', methods=['POST'])
def create_product():
    print("creando producto")
    data = request.json
    new_product = Products(name=data['name'], price=data['price'], stock=data['stock'])
